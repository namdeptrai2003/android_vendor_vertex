...
function breakfast()
{
    target=$1
    local variant=$2
    VERTEX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/vertex/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var VERTEX_VERSION)
        ZIPFILE=vertex-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.vertex.device | grep -q "$VERTEX_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $VERTEX_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.vertex.device | grep -q "$VERTEX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/vertex/build/tools/repopick.py $@
}
