...
function setup_vendor() {
    ...
    export VERTEX_ROOT="$3"
    if [ ! -d "$VERTEX_ROOT" ]; then
        echo "\$VERTEX_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$VERTEX_ROOT/$OUTDIR" ]; then
        mkdir -p "$VERTEX_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$VERTEX_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$VERTEX_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$VERTEX_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$VERTEX_ROOT"/vendor/vertex/build/tools/smali/baksmali.jar
        export SMALIJAR="$VERTEX_ROOT"/vendor/vertex/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$VERTEX_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$VERTEX_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$VERTEX_ROOT"/"$OUTDIR"/proprietary
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            if [ -f "$SRC/$FILE" ]; then
                cp "$SRC/$FILE" "$DEST"
            # if file does not exist try VERTEX target
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$VERTEX_ROOT"/"$OUTDIR"/radio
